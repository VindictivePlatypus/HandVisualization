...
public class PlayHandPoseFromFile : MonoBehaviour {
	...
	void Update ()
    {
	    ...
	    if (idx < maxIdx)
        {
            print(poses[idx, 8 * 4 + 1]);
            parts[0].localRotation = new Quaternion(poses[idx, 0 * 4 + 1], poses[idx, 0 * 4 + 2], -poses[idx, 0 * 4 + 3], poses[idx, 0 * 4]);
            //parts[1].localRotation = new Quaternion(-poses[idx, 1 * 4 + 1], poses[idx, 1 * 4 + 2], -poses[idx, 1 * 4 + 3], poses[idx, 1 * 4]);
            //parts[2].localRotation = new Quaternion(poses[idx, 2 * 4 + 1], poses[idx, 2 * 4 + 2], poses[idx, 2 * 4 + 3], poses[idx, 2 * 4]);
    ...
    void LoadFile()
    {
        ...
        poses = new float[lines.Length, 64];
